<?php
  ...
  if(isset($_POST['login-submit'])){
    ...
    if(empty($usuario)||empty($contraseña)){
      header("Location: http://localhost:8888/DesarrolloSoftware/login.php?error=camposVacios"); #mejorar
      ...
      if(!mysqli_stmt_prepare($stmt, $sql)){
        header("Location: http://localhost:8888/DesarrolloSoftware/login.php?error=sqlerror");
        ...
        if($row = mysqli_fetch_assoc($result)){
            ...
            if($pwdCheck==false){
              header("Location: http://localhost:8888/DesarrolloSoftware/login.php?error=wrongPassword");
              ...
              header("Location: http://localhost:8888/DesarrolloSoftware/inicio.php?login=success");
              exit();
            }else{
                header("Location: http://localhost:8888/DesarrolloSoftware/login.php?error=unexpectedError");
          ...
          header("Location: http://localhost:8888/DesarrolloSoftware/login.php?error=nouser");
    ...
    header("Location http://localhost:8888/DesarrolloSoftware/login.php");
  }
